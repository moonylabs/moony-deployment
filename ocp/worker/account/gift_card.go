...
func (p *runtime) giftCardAutoReturnWorker(runtimeCtx context.Context, interval time.Duration) error {
	delay := interval
...
func (p *runtime) maybeInitiateGiftCardAutoReturn(ctx context.Context, accountInfoRecord *account.Record) error {
	log := p.log.With(
...
// todo: This probably belongs somewhere more common
func InitiateProcessToCleanupGiftCardAutoReturn(ctx context.Context, data ocp_data.Provider, giftCardVaultAccount *common.Account) error {
	return data.ExecuteInTx(ctx, sql.LevelDefault, func(ctx context.Context) error {
		autoReturnAction, err := data.GetGiftCardAutoReturnAction(ctx, giftCardVaultAccount.PublicKey().ToBase58())
		if err != nil {
			return err
...
func MarkAutoReturnCheckComplete(ctx context.Context, data ocp_data.Provider, record *account.Record) error {
...
func scheduleAutoReturnAction(ctx context.Context, data ocp_data.Provider, actionRecord *action.Record, balance uint64) error {
